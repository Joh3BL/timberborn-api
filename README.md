...
4. If you plan on contributing, you can use a `main.py` file,
   as then git won't upload your *test code*.
...
# You can turn this off with a Config object at initialization, with 'start_adapter_thread=False'.
```
...
As `not_` returns a list whenever you have multiple arguments, you can use a `not_` gate to simulate *reverse gates*, like **NAND** and **NOR**. Remember to use <b>\*</b>​api.not\_ so that the list gets unpacked into *\*args*. Not\_ returns just a boolean when only 1 value is given. <!-- markdownlint-disable-line MD033 -->
...
### Lever and Adapter objects

Lever and adapter objects can both be created using the wrappers - `api.L()` & `api.A()` - and by using `api.get_lever`/`api.get_adapter`. The levers contain a *name* property, a settable *state* propety and a regular spring_return property, that can only be accessed.
...
The doscstrings also have notes on how they are intended to be used.
If something isn't provided, please open an [**Issue**](https://github.com/Joh3BL/timberborn-api/issues).

## Configuration

You usually access the api using `api = TimberbornAPI()`. All examles will also do this. To configure it, look at the initialization arguments below:
...
If that is too complicated, or it's just a simple function add, you can add it as an issue labled *enhancement*.
